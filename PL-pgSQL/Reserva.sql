...
BEGIN
    ...
    -- Selecciona la clave primaria del método de pago utilizado
    K_TARJETA_PAGO_L := PARQUEADERO.RECUPERAR_LLAVE_TARJETA_FU(ULTIMOS_CUATRO_DIGITOS_P, TIPO_TARJETA_P, NOMBRE_DUENIO_TARJETA_P, APELLIDO_DUENIO_TARJETA_P, K_CLIENTE_L);
    ...
    IF PUNTOS_USADOS_P IS NOT NULL AND PUNTOS_USADOS_P > 0 THEN
        K_FIDELIZACION_L := PARQUEADERO.RECUPERAR_LLAVE_FIDELIZACION_FU(K_CLIENTE_L);
    END IF;
    
    -- Selecciona la PK de la sucursal y el slot de parqueadero a utilizar.
    SELECT S.K_SUCURSAL,
        SP.K_SLOT_PARQUEADERO INTO STRICT K_SUCURSAL_L,
...
EXCEPTION
    ...
    WHEN NO_DATA_FOUND THEN
        RAISE EXCEPTION 'Error al recuperar la información de la sucursal. Es posible que no existan cupos para ese día o la sucursal no esté abierta para el horario ingresado.';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS 
            CODIGO_ERROR_L := RETURNED_SQLSTATE,
            RESUMEN_ERROR_L := MESSAGE_TEXT,
