...
-- Muestra la cantidad total de slot de parqueaderos más la información de la reserva.
SELECT R.FECHA_INICIO_RESERVA FECHA_RESERVA,
    EXTRACT(HOUR FROM R.FECHA_INICIO_RESERVA) HORA,
    EXTRACT(MINUTE FROM R.FECHA_INICIO_RESERVA) MINUTO,
    C.NOMBRE_CIUDAD CIUDAD,
    S.NOMBRE_SUCURSAL SUCURSAL,
    (SELECT COUNT(*) 
    FROM PARQUEADERO.SLOT_PARQUEADERO SP 
    WHERE SP.K_SUCURSAL = S.K_SUCURSAL) CANTIDAD_SLOT_PARQUEADERO
FROM PARQUEADERO.PAIS P 
    INNER JOIN PARQUEADERO.DEPARTAMENTO DP ON P.K_PAIS = DP.K_PAIS
    INNER JOIN PARQUEADERO.CIUDAD C ON DP.K_DEPARTAMENTO = C.K_DEPARTAMENTO
    INNER JOIN PARQUEADERO.DIRECCION D ON C.K_CIUDAD = D.K_CIUDAD
    ...
    INNER JOIN PARQUEADERO.HORARIO_SUCURSAL HS ON S.K_SUCURSAL = HS.K_SUCURSAL
    INNER JOIN PARQUEADERO.DIA_SEMANA DS ON HS.K_DIA_SEMANA = DS.K_DIA_SEMANA
    LEFT JOIN PARQUEADERO.RESERVA R ON S.K_SUCURSAL = R.K_SUCURSAL
WHERE R.FECHA_INICIO_RESERVA IS NOT NULL
GROUP BY R.FECHA_INICIO_RESERVA,
    C.NOMBRE_CIUDAD,
    S.NOMBRE_SUCURSAL,
    S.K_SUCURSAL;
